...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Renewable food slots across all four types (new worlds)",
    )
    parser.add_argument(
        "--brain",
    ...
    parser.add_argument("--name", help="Readable name for this run")
    parser.add_argument("--save-dir", type=Path, default=Path("saves"), help="Desktop run folder")
    parser.add_argument("--save-path", type=Path, help=argparse.SUPPRESS)
    ...
    try:
        ...
        if args.headless:
            ...
            sim = run(sim, args.save_path or args.save_dir / "world.json")
        if args.save:
            save(sim, args.save)
            export_csv(sim, args.save)
